 ...
 */
public final class ChatHandler2 {
    ...
    private static final Pattern NPC_FORGROUND_PATTERN =   Pattern.compile("^§7\\[\\d+\\/\\d+\\](?:§.)? ?§[25] ?.+: ?§..*$");
    private static final Pattern NPC_BACKGROUND_PATTERN =  Pattern.compile("^§8\\[\\d+\\/\\d+\\] .+: ?§..*$");
    ...
    private List<String> collectedLines = new ArrayList<>();
    ...
    private void handleIncomingChatLine(String message) {

        // This is a normal one line chat, or we pass a chat screen through
        Component updatedMessage = postChatLine(message, MessageType.FOREGROUND);
    ...
    private void handleWithSeparation(String message) {
       ...
       // StyledText styledText = StyledText.fromComponent(message);
        ...
        if (message.contains("\n")
                || (message.isEmpty() && (currentTicks <= chatScreenTicks + CHAT_SCREEN_TICK_DELAY))) {
            // This is a "chat screen"
            LinkedList<String> lines =getChatMessages(message);
    ...
    private void handleFakeChatLine(Component message) {
        ...
        if (isNpcLine(styledText, MessageType.FOREGROUND) == RecipientType.NPC) {
     ...
     * message entirely.
     */
    private Component postChatLine(String plainText, MessageType messageType) {
        ...
        RecipientType recipientType = isNpcLine(plainText, messageType);
    ...
    /**
     * Formerly known as getRecipientType
     * @param codedMessage
     * @param messageType
     * @return
     */
    private boolean isNpcLine(String message, MessageType messageType) {

        if (messageType == MessageType.FOREGROUND) {
            return NPC_FORGROUND_PATTERN.matcher(message).find();
        }
        return NPC_FORGROUND_PATTERN.matcher(message).find();
